...
export default function Edit({ classrooms, materials, assigment }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT',
        class_id: assigment.class_id.toString(),
        material_id: assigment.material_id?.toString() || '',
        title: assigment.title,
        description: assigment.description,
        deadline: formatDeadline(assigment.deadline),
        submission_types: assigment.submission_types || ['text', 'file'],
        attachment: null,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('guru.assigments.update', assigment.id), {
            forceFormData: true,
            onSuccess: () => {
    ...
    return (
        <DashboardLayout>
            <Head title={`Edit Tugas - ${assigment.title}`} />
            <form
                        ...
                        <div className="space-y-2 md:space-y-4">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-xs font-medium text-slate-500 md:mt-2 md:text-base lg:text-lg">
                                    Perbarui rincian penugasan untuk
                                    menyesuaikan dengan progres belajar siswa.
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-6 lg:order-1 lg:col-span-8">
                            <Card className="overflow-hidden border-none shadow-2xl shadow-slate-200/50 ring-1 ring-slate-100">
                                ...
                                <div className="border-t border-slate-50 bg-slate-50/20 p-6 md:p-12 lg:p-16">
                                    ...
                                    {assigment.attachment_path &&
                                        !data.attachment && (
                                            <div className="mt-6 flex items-center justify-between rounded-2xl border bg-slate-50 p-4 md:mt-10 md:rounded-[2rem] md:p-8">
                                                <div className="flex items-center gap-3 md:gap-6">
                                                    <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-200 text-slate-500 md:h-16 md:w-16 md:rounded-2xl">
                                                        <FileText className="h-6 w-6 md:h-8 md:w-8" />
                                                    </div>
                                                    <div className="max-w-[150px] truncate sm:max-w-xs md:max-w-none">
                                                        <p className="truncate text-xs font-bold text-slate-500 md:text-sm">
                                                            Berkas Saat Ini:
                                                        </p>
                                                        <a
                                                            href={`/storage/${assigment.attachment_path}`}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="truncate text-sm font-black text-primary hover:underline md:text-xl"
                                                        >
                                                            {assigment.attachment_path
                                                                .split('/')
                                                                .pop()}
                                    ...
                                    {data.attachment && (
                                        <div className="mt-6 flex items-center justify-between rounded-2xl border-2 border-primary/10 bg-white p-4 shadow-xl shadow-slate-100 md:mt-10 md:rounded-[2rem] md:p-8">
                                            <div className="flex items-center gap-3 md:gap-6">
                                                ...
                                                <div className="max-w-[150px] truncate sm:max-w-xs md:max-w-none">
                                                    <p className="truncate text-xs font-bold text-emerald-500 md:text-sm">
                                                        Berkas Baru Dipilih:
                                                    </p>
                        ...
                        <div className="hidden items-center gap-6 xl:flex">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-medium text-slate-500">
                                    Perubahan akan segera terlihat oleh siswa
                                    yang terdaftar.
                                </p>
                            </div>
                        </div>
