...
class AssigementController
{
    public function index(): Response
    {
        ...
        $assignments = Assigment::query()->with(['teacher', 'classroom', 'submissions' => function ($q) use ($studentId) {
            ...
            ->map(function ($assignment) use ($now) {
                ...
                return [
                    ...
                    'deadline' => $assignment->deadline->translatedFormat('d F Y'),
                    'status' => $status,
    ...
    public function show(Assigment $assigment): Response
    {
        ...
        return Inertia::render('Siswa/Assigement/Show', [
            'assigement' => [
                ...
                'deadline' => $assigment->deadline->translatedFormat('d F Y'),
    ...
    public function store(Request $request, Assigment $assigment): RedirectResponse
    {
        Gate::authorize('submit', $assigment);
        ...
        $existingSubmission = $assigment->submissions()->where('student_id', $student->id)->first();
    ...
    public function success(Assigment $assigment): Response|RedirectResponse
    {
        Gate::authorize('view', $assigment);
